...
func main() {
	var (
		...
		userID     = flag.Uint("user-id", 0, "Encrypt only memories for specific user (0 for all users)")
	)
	flag.Parse()
	...
	if !cfg.Encryption.Enabled {
		logger.Fatal().Msg("Encryption is not enabled in configuration")
	}
	...
	logger.Info().
		...
		Uint("user_id", *userID).
		Msg("Starting encryption migration")

	// Run migration
	ctx := context.Background()
	encrypted, err := runMigration(ctx, db.DB(), encryptionService, logger, *dryRun, *batchSize, *userID)
	if err != nil {
...
func runMigration(ctx context.Context, db *gorm.DB, encSvc *utils.EncryptionService, logger zerolog.Logger, dryRun bool, batchSize int, specificUserID uint) (int, error) {
	...
	for {
		// Build query for unencrypted memories
		...
		if err := query.Find(&memories).Error; err != nil {
			return totalEncrypted, fmt.Errorf("failed to fetch memories: %w", err)
		}
		...
		for _, memory := range memories {
			...
			if dryRun {
				logger.Info().
					...
					Int("content_length", len(memory.Content)).
					Msg("Would encrypt memory")
				totalEncrypted++
				continue
			}

			// Encrypt the content
			encryptedData, err := encSvc.EncryptField(memory.Content)
			if err != nil {
				logger.Error().
					Err(err).
					Uint("id", memory.ID).
					Msg("Failed to encrypt memory")
				continue
			}
			...
			if err != nil {
				logger.Error().
					Err(err).
					Uint("id", memory.ID).
					Msg("Failed to marshal encrypted data")
				continue
			}

			// Update the memory record
			updates := map[string]interface{}{
				"encrypted_content": encryptedJSON,
				"is_encrypted":      true,
				"content":           "[encrypted]",
			}
